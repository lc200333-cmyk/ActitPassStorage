...
class SpbWalletAttachmentCodec {
  ...
  SpbAttachmentPayload decode(Object? encrypted) {
    ...
    if (decrypted.length <= 4) {
      throw const SpbWalletAttachmentException(
          'Вложение SPB Wallet не содержит zlib payload.');
    }
    final expectedLength =
        ByteData.sublistView(decrypted, 0, 4).getUint32(0, Endian.little);
    try {
      final bytes =
          Uint8List.fromList(zlib.decode(Uint8List.sublistView(decrypted, 4)));
      if (bytes.length != expectedLength) {
        throw SpbWalletAttachmentException(
            'Размер вложения SPB Wallet не совпал: ожидалось $expectedLength, получено ${bytes.length}.');
      }
      return SpbAttachmentPayload(bytes);
    } catch (error) {
      throw SpbWalletAttachmentException(
          'Не удалось распаковать вложение SPB Wallet: $error');
    }
  }
  ...
  Uint8List encode(List<int> bytes) {
    ...
    ByteData.sublistView(payload, 0, 4)
